...
def hex_to_luminance(hex_color: str) -> float:
    """Compute relative luminance of a hex color for contrast checking."""
...
def has_sufficient_contrast(color_a: str, color_b: str, min_diff: float = 0.2) -> bool:
    """Check if two colors have enough luminance difference."""
    return abs(hex_to_luminance(color_a) - hex_to_luminance(color_b)) >= min_diff
...
def render_canvas(style_index: int, style: Dict[str, object], width: float, height: float) -> str:
    ...
    return f'  <rect width="{width}" height="{height}" fill="{background}"/>'
...
def orthogonalize(points: Sequence[Point]) -> List[Point]:
    """Insert elbow points so every segment is axis-aligned (horizontal or vertical)."""
    if len(points) < 2:
        return list(points)
    result = [points[0]]
...
def build_orthogonal_route(
    ...
    if arrow_data.get("route_points"):
        ...
        all_points = [start] + [(float(x), float(y)) for x, y in raw_points] + [end]
        return simplify_points(orthogonalize(all_points))

    sx, sy = start
    ex, ey = end
    # Snap nearly-aligned endpoints to eliminate cosmetic zigzag
    snap_threshold = 20.0
    if abs(sx - ex) < snap_threshold and abs(sy - ey) > snap_threshold:
        mid = round((sx + ex) / 2, 2)
        sx, ex = mid, mid
    elif abs(sy - ey) < snap_threshold and abs(sx - ex) > snap_threshold:
        mid = round((sy + ey) / 2, 2)
        sy, ey = mid, mid
    start = (sx, sy)
    end = (ex, ey)
    ...
    if best_route is not None:
        return simplify_points(orthogonalize(best_route))

    # Fallback: try wider rail routes before giving up
    wide_offsets = [48, 72, 96]
    for offset in wide_offsets:
        outer_routes = [
            [start, inner_start, (left_rail - offset, ssy), (left_rail - offset, eey), inner_end, end],
            [start, inner_start, (right_rail + offset, ssy), (right_rail + offset, eey), inner_end, end],
            [start, inner_start, (ssx, top_rail - offset), (eex, top_rail - offset), inner_end, end],
            [start, inner_start, (ssx, bottom_rail + offset), (eex, bottom_rail + offset), inner_end, end],
        ]
        for candidate in outer_routes:
            simplified = simplify_points(candidate)
            if not route_collides(simplified, expanded):
                return simplified
...
def render_label_badge(x: float, y: float, text: str, style: Dict[str, object]) -> str:
    width = max(36, estimate_text_width(text, 12.0) + 14)
...
def render_section(container: Dict[str, object], style: Dict[str, object]) -> str:
    ...
    section_radius = 4 if style_value(style, "name") in ("Notion Clean", "Enterprise") else 16
    rx = to_float(container.get("rx", section_radius))
...
def legend_layout(data: Dict[str, object], legend: Sequence[Dict[str, object]], width: float, height: float) -> Optional[Tuple[float, float, Bounds]]:
    ...
    max_label_px = max((estimate_text_width(str(item.get("label", "")), 12.0) for item in legend), default=60)
    block_width = 40 + max_label_px + 12
...
def render_rect_node(node: Dict[str, object], style: Dict[str, object], kind: str) -> str:
    ...
    elif node.get("glow") and not style_value(style, "name") == "Enterprise":
    ...
    if kind == "double_rect":
        ...
        is_enterprise = style_value(style, "name") == "Enterprise"
        header_fill_default = "#e0e0e0" if is_enterprise else "#1f2937"
        lines.append(
            f'  <rect x="{x}" y="{y}" width="{width}" height="18" rx="{rx}" fill="{node.get("header_fill", header_fill_default)}" opacity="0.95"/>'
        )
        dots_default = ["#888888", "#aaaaaa", "#666666"] if is_enterprise else ["#ef4444", "#f59e0b", "#10b981"]
        header_colors = node.get("header_dots", dots_default)
        for idx, color in enumerate(header_colors):
            lines.append(f'  <circle cx="{x + 16 + idx * 14}" cy="{y + 9}" r="4" fill="{color}"/>')
        prompt_default = "#333333" if is_enterprise else "#10b981"
        lines.append(
            f'  <text x="{x + 18}" y="{y + 44}" font-size="28" font-weight="700" fill="{node.get("prompt_fill", prompt_default)}">$</text>'
        )
        lines.append(
        ...
        for idx in range(4):
            line_y = y + 26 + idx * 14
            lines.append(
                f'  <line x1="{x + 18}" y1="{line_y}" x2="{x + width - 28}" y2="{line_y}" stroke="{node.get("line_stroke", style_value(style, "section_stroke"))}" stroke-width="1.2"/>'
            )
    elif kind == "folder":
    ...
    if kind == "bot":
        ...
        cx = x + width / 2
        cy = y + height / 2 + 2
        body_fill = node.get("body_fill", "#e5e5e5" if is_enterprise else "#1e293b")
        accent = node.get("accent_fill", "#333333" if is_enterprise else "#34d399")
        body_stroke = "#1a1a1a" if is_enterprise else "#334155"
        screen_fill = "#ffffff" if is_enterprise else "#0f172a"
        screen_stroke = "#999999" if is_enterprise else "#475569"
        mouth_fill = "#999999" if is_enterprise else "#334155"
        lines.append(f'  <rect x="{cx - 42}" y="{cy - 32}" width="84" height="84" rx="18" fill="{body_fill}" stroke="{body_stroke}" stroke-width="1.8"{filter_attr}/>')
        lines.append(f'  <rect x="{cx - 26}" y="{cy - 16}" width="52" height="22" rx="6" fill="{screen_fill}" stroke="{screen_stroke}" stroke-width="1.2"/>')
        lines.append(f'  <circle cx="{cx - 12}" cy="{cy - 5}" r="5" fill="{accent}"/>')
        lines.append(f'  <circle cx="{cx + 12}" cy="{cy - 5}" r="5" fill="{accent}"/>')
        lines.append(f'  <rect x="{cx - 14}" y="{cy + 14}" width="28" height="6" rx="3" fill="{mouth_fill}"/>')
    ...
    # Add clipPath for text inside node box (skip for special kinds with external text)
    needs_clip = kind not in {"document", "folder", "circle_cluster", "bot", "terminal", "user_avatar"}
    if needs_clip:
        clip_id = f"clip-{round(x)}-{round(y)}"
        lines.append(f'  <clipPath id="{clip_id}"><rect x="{x + 2}" y="{y + 2}" width="{width - 4}" height="{height - 4}" rx="{max(rx - 2, 0)}"/></clipPath>')
...
def render_node(node: Dict[str, object], style: Dict[str, object]) -> str:
    ...
    if kind == "cylinder":
        ...
        clip_id = f"clip-{round(x)}-{round(y)}"
        lines.append(f'  <clipPath id="{clip_id}"><rect x="{x + 2}" y="{y + ry}" width="{width - 4}" height="{height - 2 * ry - 4}"/></clipPath>')
        lines.append(f'  <g clip-path="url(#{clip_id})">')
        lines.append(f'  <text x="{x + width / 2}" y="{y + height / 2 - 6}" text-anchor="middle" class="node-title">{label}</text>')
        if subtitle:
...
def render_legend(
    ...
    if data.get("legend_box"):
        ...
        if is_enterprise:
            bg = "#f5f5f5"
            opacity = 1.0
            lines.insert(0, f'  <rect x="{legend_x - 10}" y="{legend_y - 14}" width="{block_width + 20}" height="{block_height + 18}" rx="2" fill="{bg}" stroke="#1a1a1a" stroke-width="1.4"/>')
        else:
...
NARROW_CHARS = set("iljtfr1!|.,:;'/(){}[]` ")
WIDE_CHARS = set("WMwmw@OQDG")

def estimate_text_width(text: str, font_size: float = 18.0) -> float:
    """Estimate pixel width for mixed CJK/Latin text at given font_size."""
    ...
    for ch in text:
        if "一" <= ch <= "鿿" or "㐀" <= ch <= "䶿" or "豈" <= ch <= "﫿":
            ...
            w += 7.0 * scale
    return w * 1.15  # 15% safety margin for font-weight, letter-spacing, kerning


def auto_compute_node_size(node_data: Dict[str, object]) -> None:
    """Adjust node width/height to fit label text without overflow. Mutates node_data."""
    kind = str(node_data.get("kind", node_data.get("shape", "rect")))
    if kind in {"circle", "cylinder"}:
        return
    title = str(node_data.get("label", ""))
    subtitle = str(node_data.get("sublabel", ""))
    type_label = str(node_data.get("type_label", ""))
    max_text_width = max(
        estimate_text_width(title, 18.0),
        estimate_text_width(subtitle, 12.0),
        estimate_text_width(type_label, 12.0),
        0.0,
    )
    min_width = max(140, max_text_width + 48)  # 24px padding each side
    current_width = to_float(node_data.get("width", 180))
    if min_width > current_width:
        node_data["width"] = round(min_width, 1)

    lines_count = sum(1 for t in [title, subtitle] if t.strip())
    if type_label.strip():
        lines_count += 1
    if node_data.get("tags"):
        lines_count += 1
    min_height = 28 + max(1, lines_count) * 22
    current_height = to_float(node_data.get("height", 76))
    if min_height > current_height:
        node_data["height"] = round(min_height, 1)


def resolve_node_overlaps(nodes_data: List[Dict[str, object]], max_iterations: int = 20) -> Tuple[float, float]:
    """Detect overlapping nodes and nudge apart in both axes. Returns (max_x, max_y) for viewBox expansion."""
    if not nodes_data:
        return 0.0, 0.0
    MIN_GAP = 24
    for _ in range(max_iterations):
        changed = False
        bounds_list = [(idx, node_bounds(node), node) for idx, node in enumerate(nodes_data)]
        for i in range(len(bounds_list)):
            _, bounds_a, node_a = bounds_list[i]
            for j in range(i + 1, len(bounds_list)):
                idx_b, bounds_b, node_b = bounds_list[j]
                if not bounds_intersect(bounds_a, bounds_b, padding=MIN_GAP):
                    continue
                # Compute overlap in each axis
                overlap_x = (bounds_a[2] + MIN_GAP) - bounds_b[0]
                overlap_y = (bounds_a[3] + MIN_GAP) - bounds_b[1]
                # Choose the axis with less overlap to nudge
                if overlap_x <= 0 and overlap_y <= 0:
                    continue
                if overlap_x <= 0:
                    # Only vertical overlap
                    node_b["y"] = round(to_float(node_b.get("y", 0)) + overlap_y, 1)
                elif overlap_y <= 0:
                    # Only horizontal overlap
                    node_b["x"] = round(to_float(node_b.get("x", 0)) + overlap_x, 1)
                elif overlap_x <= overlap_y:
                    node_b["x"] = round(to_float(node_b.get("x", 0)) + overlap_x, 1)
                else:
                    node_b["y"] = round(to_float(node_b.get("y", 0)) + overlap_y, 1)
                bounds_list[j] = (idx_b, node_bounds(node_b), node_b)
                bounds_b = bounds_list[j][1]
                changed = True
        if not changed:
            break
    # Compute max extent
...
def build_svg(template_type: str, data: Dict[str, object]) -> str:
    ...
    # Hard layout constraints: auto-size nodes to fit text, then resolve overlaps
    for node in nodes_data:
        auto_compute_node_size(node)
    max_node_x, max_node_y = resolve_node_overlaps(nodes_data)

    # Auto-expand viewBox if nodes exceed bounds
    MARGIN = 60
    if max_node_x + MARGIN > width:
        width = max(width, max_node_x + MARGIN)
    if max_node_y + MARGIN > height:
        height = max(height, max_node_y + MARGIN)
    ...
    # Arrow labels rendered before nodes so nodes visually cover any overlap
...
def main() -> None:
    ...
    try:
        if len(sys.argv) > 3:
            ...
            # Force UTF-8 for stdin to avoid Windows codec issues with CJK input
            stdin_stream = sys.stdin
            if hasattr(sys.stdin, "buffer"):
                stdin_stream = io.TextIOWrapper(sys.stdin.buffer, encoding="utf-8")
            data = json.load(stdin_stream)
